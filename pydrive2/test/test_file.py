...
class GoogleDriveFileTest(unittest.TestCase):
  ...
  def test_03_Files_Insert_Content_String(self):
    ...
    pydrive_retry(lambda: file1.FetchContent())  # Force download and double check content
    self.assertEqual(file1.metadata['title'], filename)
    self.assertEqual(file1.GetContentString(), content)
  ...
  def test_Files_UnTrash_File_Just_ID(self):
    ...
    file2 = drive.CreateFile({'id': file1['id']})
    pydrive_retry(lambda: file2.UnTrash())  # UnTrash without fetching metadata.
  ...
  def test_GFile_Conversion_Lossless_String(self):
    ...
    pydrive_retry(lambda: file1.GetContentFile(downloaded_file_name, mimetype='text/plain'))
  ...
  def test_GFile_Conversion_Remove_BOM(self):
    ...
    try:
      ...
      # Download as file.
      pydrive_retry(lambda: file1.GetContentFile(downloaded_file_name, remove_bom=True))
  ...
  def _parallel_uploader(self, num_of_uploads, num_of_workers,
    ...
    files = pydrive_retry(
        lambda: drive.ListFile(param={'q': "title = '%s' and trashed = false" %
                               remote_name}).GetList()
    )
  ...
  @pytest.mark.skipif(sys.platform == 'win32', reason="timeout_decorator doesn't support Windows")
