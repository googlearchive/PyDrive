...
class GoogleAuth(ApiAttributeMixin, object):
    ...
    def __init__(self, settings_file='settings.yaml'):
        ...
        :param settings_file: path of settings file.
                              'settings.yaml' by default.
    ...
    def LocalWebserverAuth(self, host_name='localhost',
        ...
        """Authenticate and authorize from user by creating local webserver
        and retrieving authentication code.

        This function is not for webserver application. It creates local
        webserver for user from standalone application.
        ...
        if success:
            ...
            print 'Failed to start a local webserver. Please check your'
            print 'firewall settings and locally running programs that'
            print 'may be blocking or using configured ports. Default'
            print 'ports are 8080 and 8090.'
        ...
        if 'code' in httpd.query_params:
            ...
            print 'parameters of the redirect.'
    ...
    def LoadClientConfig(self, backend=None):
        ...
        If you have any specific backend to load client configuration from
        in mind, don't use this function and use the corresponding function
        you want.
        ...
        if backend is None:
            ...
            if backend is None:
                raise InvalidConfigError('Please specify client \
                                         config backend')
        if backend == 'file':
        ...
        for config in self.CLIENT_CONFIGS_LIST:
            try:
                self.client_config[config] = \
                    self.settings['client_config'][config]
            except KeyError:
                raise InvalidConfigError('Insufficient client \
                                         config in settings')
    ...
    def GetFlow(self):
        ...
        self.flow = \
            OAuth2WebServerFlow(self.client_config['client_id'],
